...
// ********************* Anpassung an DFPlayer von DF-ROBOT mit ILSP3 Chip *****************************************
// ---------------- Nutzung eines DF-Players, der bei Adverts ein EndOfTrack Event ausgibt --- Nur TonUINO Classic -
#ifndef AiO                     // Nur TonUINO Classic
  #define DfPlayer_spez         // Einige DF-Player geben beim Abspielen von Advert-Tracks ein EndOfTrack Event aus.
...
// ---------------------Jackdetekt-Kopfhörer --- Nur TonUINO Classic --- Nutzung mit der AiO nach Hardwareänderung möglich ---
...
uint8_t voiceMenu(                                 // Rückgabewert voiceMenu
  ...
  int startMessage,                                // Tracknr der StartAnsage im mp3Ordner
...
// ****************Sperre der Tasten und RFID durch ModKarte in Konsole anzeigen ********************
void ShowLockPause()
{
#ifdef Konsole
  Serial.println(F("Pause -> LOCKED!"));    // Anzeige - Sperre Play/Pausetaste
#endif
}
// -------------------------------------
void ShowLockNext()
{
#ifdef Konsole
  Serial.println(F("Next -> LOCKED!"));     // Anzeige - Sperre Nächster Titel
#endif
}
// -------------------------------------
void ShowLockPrevious()
{
#ifdef Konsole
  Serial.println(F("Prev. -> LOCKED!"));    // Anzeige - Sperre Vorhergehender Titel
#endif
}
// -------------------------------------
void ShowLockVolUp()
{
 #ifdef Konsole
  Serial.println(F("Vol.Up -> LOCKED!"));   // Anzeige - Sperre Lautertaste
 #endif
}
// -------------------------------------
void ShowLockVolDown()
{
#ifdef Konsole
  Serial.println(F("Vol.Down -> LOCKED!")); // Anzeige - Sperre Leisertaste
#endif
}
// -------------------------------------
void ShowLockRFID()
{
#ifdef Konsole
  Serial.println(F("RFID -> LOCKED!"));     // Anzeige - Sperre Neue Karte abspielen
#endif
}
...
class SleepTimer : public Modifier
{
  ...
  public:
    ...
    SleepTimer(float minutes)                        // Variablentyp auf float geändert für Werte < 1
    {
      ...
      this->lastVolCheck = millis();
#ifdef BatteryCheck
      if (!BattLeer)                                // Wenn Batterie nicht leer -> Ansage der Schlummerfunktion
      {
#endif
        delay(500);
    ...
    /*  // ---------------------------------------------------------------
        virtual bool handlePause()
        {
          ShowLockPause();           // Play/Pausetaste sperren
          return true;
        }
    */
    /*// ---------------------------------------------------------------
    virtual bool handleNextButton()
    {
      ShowLockNext();             // Next Taste sperren
      return true;
    }
    // ---------------------------------------------------------------
    virtual bool handlePreviousButton()
    {
      ShowLockPrevious();         // Zurück Taste sperren
      return true;
    }
    // ---------------------------------------------------------------
    /*virtual bool handleVolumeUp()
      {
      ShowLockVolUp();         // Lautertaste sperren
      return true;
      }
      // ---------------------------------------------------------------
      virtual bool handleVolumeDown()
      {
      ShowLockVolDown();      // Leisertaste sperren
      return true;
      }
      // ---------------------------------------------------------------
    */
    virtual bool handleRFID(nfcTagObject *newCard)
    {
      ShowLockRFID();           // Neue Karte abspielen sperren
      return true;
    }
    // ---------------------------------------------------------------
    uint8_t getActive()
...
// --------------------------------------------------------------------
    /*    virtual bool handlePause()
          {
            ShowLockPause();                            // Pausetaste gesperrt
            return true;      
          }
    */
// --------------------------------------------------------------------
    virtual bool handleNextButton()
    {
      ShowLockNext();                                   // VorTaste gesperrt
      return true;
    }
// --------------------------------------------------------------------
    virtual bool handlePreviousButton()
    {
      ShowLockPrevious();                               // ZurückTaste gesperrt
      return true;
    }
// --------------------------------------------------------------------
    virtual bool handleRFID(nfcTagObject * newCard)
...
// --------------------------------------
    case 5: //Hoerbuch
#ifdef Konsole
     if (ResetHB) Serial.println ("next Track und Wiederholung Epromupdate nach HB Reset gesperrt");
#endif
//#ifdef Konsole
        Serial.print("idx: ");
        Serial.print(currentQueueIndex);
        Serial.print(" --> ");
//#endif
     if (!ResetHB)                                          // Wenn nxtTrk und update EEProm HB nicht gesperrt
     { 
...
void volumeUpButton()
{
  if (activeModifier != NULL)                          // wenn Modifikation aktiv
    if (activeModifier->handleVolumeUp())              // wenn Taste gesperrt
      return;                                          // kein VolUp

#ifdef SpkOnOff
  if (SpkisOn)                                         // wenn Lautsprecher an
  {
#endif
    //#ifdef Konsole
    if (volume >= mySettings.maxVolume )               // wenn Vol größer oder gleich maxVolume Spk
    {
      if (msgMax)                                      // wenn max Volume erreicht
      {
    ...
    {
      if (longPressVol)                               // VolÄnderung durch longPress
      {  
...
{
  if (activeModifier != NULL)                          // wenn Modifikation aktiv
    if (activeModifier->handleVolumeDown())            // wenn Taste gesperrt
      return;                                          // kein VolDown
...
{
  if (activeModifier != NULL)                          // wenn Modifikation aktiv
    if (activeModifier->handleNextButton())            // wenn Taste gesperrt
...
{
  if (activeModifier != NULL)                           // wenn Modifikation aktiv
    if (activeModifier->handlePreviousButton())         // wenn Taste gesperrt
...
#ifdef LastCard
  if (!ShortCutIsPlaying)                                 // wenn Marker  "shortcut wird nicht gespielt"
  {
#ifndef AiO                                               // TonUINO Classic
...
#endif
  if (!fromCard)                               // wenn Adminmenü über Tasten aufgerufen
  { 
  ...
  do
  {
    ...
    if (ButtonTwo.pressedFor(LONG_PRESS))                               // longpress Taste 2 (Konfigurationsabhängig Weiter oder Lauter)
    {
     ...
     {                                                                  // soll trotzdem nur einmal Longpress ausgewertet werden
      ...
      if (rolling)                                                      // wenn rolling erlaubt
          {
    ...
    if (ButtonThree.pressedFor(LONG_PRESS))                     // Longpress Taste 3 (Zurück oder Leisertaste, Konfigurationsabhängig)
    {
     ...
     {                                                          // soll trotzdem nur einmal Longpress ausgewertet werden
      ...
      if (!rolling)                                             // wenn rolling gesperrt   
        {
 ...
 if(Ans)
{
        ...
        if (! AdjVol) Serial.println(returnValue);                  // Nr der Option
...
void wecker()
{
  ...
  if (!ActWecker)                               // Wenn Wecker noch nicht aktiviert wurde
  {
    ReadWecker = analogRead(WeckerPin);         // Weil Analogpin 6 und 7 nicht als Digitalpins verwendet werden können
  }                                             // muss ein Umweg über analogRead() gegangen werden.
#ifndef AiO
  if ((ReadWecker >= 512)||WeckerStart)         // wenn Analogwert WeckerPin > 2,5V (TonUINO Classik)
#endif
#ifdef AiO
  if ((ReadWecker >= 1500)||WeckerStart)        // wenn Analogwert WeckerPin > 1,6V (AiO )
#endif
  {
//#ifdef Konsole
...
//#endif
  ...
  else                                          // Wecker wurde nicht aktiviert
  {
    ...
    if (SETUP)                                  // wenn Programmablauf im Setup
    {
...
{
#ifdef Wecker
  if (WeckerPlay)                            // Marker Weckershortcut wird gespielt gesetzt?
  {
  ...
  if (!WeckerPlay)                              // Weckershortcut wird nicht gespielt
  {
#endif
...
{
#ifdef EarPhone
  if (!SpkisOn) EarVol = volume ;                              // KH-Lautstärke speichern
...
void LED_Animation()
{
  ...
  if (lsrEnable)                        // Wenn wahr, Animationen durchführen
  {
#endif
 ...
 //*************LED-Animaion by @tON ****  modifyed by @atomphil ***********************
  ...
  if (lsrAudioBreak)           // wenn Wiedergabe von LED On/Off gestoppt
  {
...
// *************************** Fehlerbehandlung des Df-Players *****************************
  void mp3Error()
  {
  if (mp3error)                                 // bei Vorliegen eines Fehlercodes vom Df-Player
    {
     if (!StdBy) setstandbyTimer();             // Standbytimer starten
...
#endif
  Serial.println();
  Serial.println(F("** VERSION THOMAS LEHNERT ** 2022-04-07 **"));
  Serial.println();
#ifndef AiO
...
myCookie = myCookie +4;                // setzt myCookie um 4 hoch (9 Zusätzliche Shortcuts in den Settings)
 ...
 //************* Ordner des Speichermediums zählen *****************
  ...
  if (SpkisOn) mp3.setVolume(SpkVol);                 //Startlautstärke Lautsprecher einstellen
 #ifdef EarPhone
...
ReadMatrix();                                 // Kontrolle ob eine Taste der Matrix gedrückt wurde.
    ...
    // ************************ Wecker **** Hardwareerweiterung erforderlich **********************
#ifdef Wecker
    if (! ActWecker)   // wenn Wecker in aktiver Session noch nicht aktiviert worden ist
      wecker();
#endif
...
if ((ButtonOne.pressedFor(3000)               // 2 Tastendruck 5 sekunden zum Ausschalten
        || ButtonThree.pressedFor(3000))
        && ButtonOne.isPressed()
        && ButtonThree.isPressed()
        && !(ButtonTwo.isPressed()))
        ShutDown();
...
// ------------- ShortPress --------------------------------
    if (ButtonOne.wasReleased())                                // wenn Play/Pausetaste gedrückt worden ist
    {
      ...
      if (activeModifier != NULL)                               // wenn Modifikation aktiv
        if (activeModifier->handlePause())                      // wenn akt.Modifikation pause Taste sperrt
          return;                                               // Abbrechen
...
// ---------------- LongPress -------------------------------------
             ...
             && !(ButtonThree.isPressed())
             && !ignoreButtonOne)                               // Langer Druck Pausetaste für Ansage des aktuellen Tracks
    {                                                           // oder Hörbuch auf Anfang zurücksetzen

      if (activeModifier != NULL)                               // wenn Modifikationskarte aktiv
        if (activeModifier->handlePause())                      // wenn Pausetaste gesperrt - Abbruch
          return;
      if (isPlaying())                                          // wenn Wiedergabe läuft
      {
 ...
 // ---- Spezialmodus Von-Bis für Album gibt die Dateinummer relativ zur Startposition wieder ----
    ...
    // ---------------- LongPress -------------------------------------
#ifdef LED_SR_Switch
    if (ButtonTwo.pressedFor(LONG_PRESS) && !lsrSwitch )
#endif
  ...
  if (readCard(&myCard))                             // wenn Karte lesbar
  {
...
/*
    ...
    - Über Longpress Taste 1 (Play) + Taste 3 (Zurück) kann der Tonuino ausgeschaltet werden.                                      - (AiO, Classic)
    - Das Buttonboard 12 Tasten wurde integriert. Durch #define Buttonboard aktivierbar.                                           - (AiO, Classic)
      Die 9 Tasten werden über das Adminmenü mit Shortcuts belegt.
    - Bei manchen DF-Playern wird auch am Ende eines Adverttracks ein Track beendet Event gesendet.                                - (TonUINO Classic)
      Das führt zu einem unbeabsichtigten NextTrack und es werden Titel übersprungen. Das wird jetzt abgefangen
      unabhängig vom Verhalten des DF-Players.(Betrifft z.B. den DFROBOT Player DFRO299)
