...
********* Always Queue - alle Hardwareversionen ************************
 ...
 Bearbeitungsstand:
*/
# define Bearbeitungsstand "2022-09-13"          // Datum der letzten Änderung
      
/*      Version Thomas Lehnert


    - Über #define wird ausgewählt, welche Hardwarekonfiguration verwendet wird. Auswahlmöglichkeiten
    sind: AiOPLUS, AiO bis Version 1.2, Klassik mit Arduino Nano oder UNO, Klassik mit Arduino EVERY.
...
// ----------------------TonUINO Klassik mit Arduino NANO 328P --automatische Auswahl -----------------------------
                         ...
                         // Automatisches setzen auf Klassik, wenn keins der obigen Boards definiert ist.
                         // Software für die Klassik-Version mit Arduino-Nano oder Uno
#endif                         
// =================================================================================================================
...
// --------------------- Debug Modus --- AiO und Klassik mit Arduino NANO oder UNO -----------------------------------
...
// -------------------- 3 Tasten Support --- Alle Versionen ----------------------------------------------------------
                              // Für den Betrieb mit drei Tasten #define FIVEBUTTONS und #define Buttonboard
                              // in Kommentar setzen
                              
// -------------------- 5 Tasten Support --- Alle Versionen ----------------------------------------------------------
//#define FIVEBUTTONS           // 5 Tasten support
                              // In diesem Modus kann die Batterieüberwachung im TonUINO Klassik nicht verwendet werden
...
// -------------------- Support des 12 Buttonboards mit 3 Standarttasten und 9 Zusatztasten für Shortcuts --- Alle Versionen 
//#define Buttonboard           // Unterstützung des Buttonboards mit 12 Tasten. 
 ...
 #endif                       
// ------------------- Sprungweite für Titelsprung Vorwärts/ Rückwärts bei LongPress Vor/Zurück --- Alle Versionen -----
...
// -------------------- Zuletzt gespielte Karte als Shortcut speichern --- Alle Vversionen -------------------------------
...
// ---------------------- Hörbuch auf Anfang zurücksetzen --- Alle Versionen ---------------------------------------------
...
// ---------------------- Lautstärke für das Adminmenü festlegen --- Alle Versionen --------------------------------------
...
// ---------------------- Zählen der Ordner auf der SD-Karte --- Alle Versionen ------------------------------------------
...
// ---------------------- Windows SystemVolumeInfo ignorieren --- Alle Versionen -----------------------------------------
...
#define HW_PowerOff            // Abschaltung über Hardware, wie Mosfet oder Pololu-Switch
  ...
  #endif                       // Für Pololu-Swich (HIGH aktiv) #define LowActive deaktivieren.

// ---------------------- Abschaltung für AiO und AiOPLUS ------------------------------------------------------------------
                               // Die Hardware zum Ein und Ausschalten ist auf der AiO und AiOPLUS bereits enthalten.
                               // Für die AiO und AiOPLUS braucht HW-PowerOff nicht konfiguriert werden
                               // Die Funktion (ShutDown) ist für AiO und AiOPLUS 
                               // in der Software richtig konfiguriert enthalten.
                               
// ---------------------- Abschaltung durch die Powerbank bei Unterschreiten der Mindestlast -- I < 27 mA -- nur Klassik ----
#ifndef AiOPLUS
#ifndef AiO                    // Nur für den TonUINO Klassik
...
#endif                         // Bei Unterschreitung der Mindestlast schaltet die Powerbank selbstständig automatisch aus.
...
// ---------------------- LED Animation mit NEOPixel LED Ring oder Strip --- Alle Versionen --------------------------------
#define LED_SR                 // LED Animation,Hardwareerweiterung erforderlich: (Neopixel LED's,Strip oder Ring)
 #ifndef AiOPLUS
 #ifndef EVERY                 // Bei TonUINO Klassik mit NANO, oder AiO, 
...
//#define UsbStick              // An Stelle der SD-Karte arbeitet der DF-Player mit einem USB-Stick
...
#ifdef UsbStick
 #ifdef AiO                    // Betrieb mit USB-Stick ist an der AiO und AiOPlus nicht möglich
  #undef UsbStick 
 #endif   
 #ifdef AiOPLUS
  #undef UsbStick             
 #endif 
#endif  
// ---------------------- Lautsprecher Abschaltung über Software --- Alle Versionen ----------------------------------
#define SpkOnOff              // Aus und Einschalten des Lautsprechers über Software 
                              ...
                              // Hardwareerweiterung für TonUINO Klassik erforderlich: (Abschaltung des Lautsprechers über MOS-FET's)
                              // Bei der AiO wird der Verstärker ein bzw. ausgeschaltet.

// -----------Jackdetekt- Kopfhörer --- AiOPLUS und TonUINO Klassik --- Nutzung mit der AiO bis V1.2 nach Hardwareänderung möglich ---
                                
//#define JackdetektAiO         // *** Jackdetekt Kopfhörer - AiO bis Vers 1.2 *** nur mit Hardwareänderung ------------------
                               // Bei gestecktem Kopfhörer wird das Jackdetekt-Signal ausgewertet 
                               ...
                               // Nur aktivieren wenn Hardwareänderung für Jackdetekt auf der AiO Platine durchgeführt wurde.
                               // Ohne HW-Änderung in Kommentar setzen !!!
 
#ifdef AiO                    // Abfrage für Jackdetekt der AiO. Hier nichts ändern!
  #ifdef JackdetektAiO        // Wenn für die AiO Jackdetekt aktiviert ist (Siehe oben)
   #define Jackdetekt         // Jackdetekt aktivieren (Option hier NICHT ändern !!!)
  #endif
#endif             
                              // *** AiOPLUS und TonUINO-Klassik *******
#ifndef AiO                   // Hier für AiOPLUS und TonUino Klassik änderbar
   #define Jackdetekt         // Jackdetekt AiOPLUS oder Klassik mit Nutzung Kopfhörer-Buchse oder Platine mit Jackdetekt.
#endif                              
...
#ifndef AiOPLUS
#ifndef AiO                   // Nur TonUINO Klassik
...
// ------------------- Lese-Empfindlichkeit des RFID-Lesers --- Alle Versionen ----------------------------------------------------
...
#endif
// ------------------- Überwachung der Batteriespannung --- Alle Versionen --------------------------------------------------------
#define BatteryCheck        // Bei AiO bis V1.2 muss die Lötbrücke SJ1 geschlossen werden und das modifizierte BSP LGT8FX
...
// Wichtiger Hinweis !!!!!     Bei Anschluss der AiOPUS oder AiO über die USB-Buchse wird in der Konsole nicht die richtige Akkuspannung angezeigt.
                            // Wenn diese über die Konsole korrekt angezeigt werden soll, (nicht im Ladebetrieb)
                            // muss der PC über einen FTDI-Adapter (Jumper auf 3,3V) an rx/tx und Ground
                            // am Erweiterungsport der AiO Platine angeschlossen werden.
//                           ___________________________________________________________________________________
 ...
 // --------------------------------------- Akkutyp festlegen --------------------------------------------------------
                                   // Die AiO und AiOPLUS Platinen sind standardmäßig für den LiFePO4 Betrieb ausgelegt,
 ...
 // -------------------------------- TonUINO Klassik --------------------------------------------------- 
 #ifndef AiOPLUS                             
  #ifndef AiO                      // TonUINO Klassik , Batteriespannung über 100kOhm an BattCheckPin anlegen
  #define Bandgap 1104000L   
  #endif                      
 #endif
 // -------------------------------- TonUINO Klassik mit Nano Every -------------------------------------- 
 #ifdef EVERY                      // Batteriespannung über 100kOhm/100kOhm Spannungsteiler an BattCheckPin anlegen
...
// ------------------------------------------------------------------------------------------------------------------------------------ 
#ifdef AiOPLUS
  #define Hardwareserial        // AiOPLUS verwendet Hardwareserial über (UART3) serial3  zum DF-Player
#endif
#ifdef EVERY
...
#endif                            // Ende PIN Deklaration AiOPLUS
...
#ifndef AiOPLUS
#ifndef AiO                         // für TonUINO Klassik und Every
...
#endif                            
#endif                            // Ende Pin-Definitionen TonUINO Klassik
...
const char* lastch = Bearbeitungsstand;         // Datum der letzten Änderung
...
const float Vshutdown = BatEmpty;               // Voltzahl für Shutdown, Batterie leer, bei den #defines festgelegt
 ...
 #endif
 #ifdef EVERY
 unsigned long TestIntervall = 1000;            // Testintervall Batterie  ,   EVERY: 1 Sekunde
 #endif
#endif
#ifdef AiO
...
#endif
#ifdef AiOPLUS
 unsigned long TestIntervall = 1000;            // Testintervall Batterie  ,   AiOPLUS: 1 Sekunde
#endif
...
//DFMiniMp3<HardwareSerial, Mp3Notify> mp3(Serial1); // Digitalpins zur Steuerung des DF-Players müssen umgelegt werden.
                                                   // D0 RX, D1 TX 
#endif
#ifndef Hardwareserial
...
// ***************** Error-meldungen des df-players **************************
    ...
    static void OnError(uint16_t returnValue)
    {
      switch (returnValue)
      {
        ...
        case DfMp3_Error_FileIndexOut: {
            Serial.print(F("5- file idx Out "));     // Dateinummer außerhalb des Bereiches
            break;
          }
        case DfMp3_Error_FileMismatch: {
            Serial.print(F("6- file n.found"));      // Datei nicht gefunden
            break;
          }
    ...
    {
      if (source & DfMp3_PlaySources_Sd) Serial.print("SD-K ");
#ifdef UsbStick
    ...
    static void OnPlaySourceOnline(DfMp3_PlaySources source)                // Abspielmedium Online
    {
      PrintlnSourceAction(source, "online");
    }

    // Speichermedium bereit
    static void OnPlaySourceInserted(DfMp3_PlaySources source)              // Abspielmedium Bereit
    {
      PrintlnSourceAction(source, "bereit");
    }

    // Speichermedium entfernt
    static void OnPlaySourceRemoved(DfMp3_PlaySources source)               // Abspielmedium nicht erkannt (entfernt)
    {
...
#ifdef Hardwareserial
#ifdef EVERY
static DFMiniMp3<HardwareSerial, Mp3Notify> mp3(Serial1);                   // Serial 1 für Klassik mit EVERY
#endif
#ifdef AiOPLUS
static DFMiniMp3<HardwareSerial, Mp3Notify> mp3(Serial3);                  // Serial 3 für AiOPLUS
#endif
#endif
#ifndef Hardwareserial
static DFMiniMp3<SoftwareSerial, Mp3Notify> mp3(mySoftwareSerial);         // Softwareserial Klassik und AiO
#endif
...
const PROGMEM char* getModeName(uint8_t mode)
{
  switch (mode)
  {
    case 1:   return PSTR("Hörsp.modus");
    case 2:   return PSTR("Alb.modus");
    case 3:   return PSTR("Pty.modus");
    case 4:   return PSTR("Einz.modus");
    case 5:   return PSTR("Hörb.modus");
    case 6:   return PSTR("Von-Bis: Hörsp.");
    case 7:   return PSTR("Von-Bis: Alb.");
    case 8:   return PSTR("Von-Bis: Pty.");
    case 9:   return PSTR("Adm.menue");
    default:  return PSTR("?");
  }
}
...
// ---------------------------------------------------------------------------------
  ...
  #ifdef LED_SR                            // LED-Animation RESET von mySettings
...
#endif
#ifdef LastCard
  Serial.println(F("SC-0,Play-Taste: Letzte Karte")); 
 #endif
#ifndef FIVEBUTTONS
  #ifndef Buttonboard
...
// ------------------------------------
  ...
  if (countTi >= 60)                                                   // Testzyklenzahl erreicht
  {
    ...
    Serial.print(VbattGl);                                
    Serial.println(" V");
    Serial1.print(F("Batt.=  "));                   // Ausgabe der Batteriesspannung über Serial1
    Serial1.print(VbattGl);                         // FTDI Adapter an RX1/TX1 und GND anschließen       
    Serial1.println(" V");                          // Jumper auf 3,3V setzen             
...
#ifdef Konsole
Serial.println (" Wait for Track To Finish");
#endif

  do                                                          // Schleife zwischen Ende Track und 1000 ms danach
  {
  ...
  do                                                          // Schleife solange Track spielt
  {
    mp3.loop();                                               // Meldung vom df-Player über serial?
  }
...
void volumeUpButton()
{
  ...
  if (SpkisOn == true)                                 // wenn Lautsprecher an
  {
    ...
    if (volume >= mySettings.maxVolume )               // wenn Vol größer oder gleich maxVolume Spk
    {
      ...
      {
      Serial.println("Max Volume !");                  // Anzeige Konsole 
...
void adminMenu(bool fromCard = false)    
{
  ...
  else if (subMenu == 2)                                      // 902- "Einzelkarten für einen Ordner festlegen"
  {
...
#ifdef Konsole
    Serial.print("Dateien im Ordner: ");
    Serial.print(tempCard.nfcFolderSettings.folder);
    Serial.print(" -> ");
    Serial.println(numTracksInFolder);
#endif  

    uint8_t special = voiceMenu(numTracksInFolder, 321, 0,true,
                      tempCard.nfcFolderSettings.folder);           // 321- "Startdatei auswählen"
    ...
    uint8_t special2 = voiceMenu(numTracksInFolder, 322, 0,true,
                       tempCard.nfcFolderSettings.folder, special); // 322- "Enddatei auswählen"
                                                                         // setzt den End-Track
...
// ********** Modifikationskarte schreiben ***********
      do                                                          // Schleife bis Karte aufgelegt, oder Abbruch
      {
        ...
        readButtons();
        if (ButtonTwo.wasReleased()||ButtonThree.wasReleased())   // Taste 2 oder 3 gedrückt für Abbruch?
        {
#ifdef Konsole
          Serial.println(F("Abbruch!"));
#endif
...
void resetCard()
{
  ...
  {
#ifdef Konsole
  Serial.print(F("Karte nicht lesbar!"));
#endif
...
bool setupFolder(folderSettings * theFolder)
{
  ...
  if (theFolder->mode == 4)
    theFolder->special = voiceMenu(numTracksInFolder, 320, 0,true,
                          theFolder->folder);                                           // 320 - Trackauswahl
  ...
  if (theFolder->mode == 6 || theFolder->mode == 7 || theFolder->mode == 8)              // Spezial Von Bis : Hörsp, Album, Party 
  {
    ...
    theFolder->special = voiceMenu(numTracksInFolder, 321, 0,true,
                          theFolder->folder);                                            // StartTrack
    ...
    theFolder->special2 = voiceMenu(numTracksInFolder, 322, 0,true,
                          theFolder->folder, theFolder->special);                        // EndTrack
...
bool readCard(nfcTagObject * nfcTag)
{
  ...
  {
#ifdef Konsole
    Serial.println(F("Authenticating Klassik using key A..."));
#endif
...
#ifndef AiO
  digitalWrite(SpkOnPin, HIGH);                               // Lautsprecher über Mosfets Einschalten (Klassik)
#endif
#ifdef AiO
...
#ifndef AiO
    digitalWrite(SpkOnPin, LOW);                                // Lautsprecher über Mosfets Ausschalten (Klassik )
#endif
#ifdef AiO
...
void setup()
{
...
#ifdef AiOPLUS
  Serial1.begin(9600);                   // serielle Schnittstelle1 für FTDI-Adapter zur Batteriespannungskontrolle initialisieren
 ...
 Serial.print(F("- EVERY "));
  ...
  Serial.println("**");
  Serial.println();
  Serial.print(F("** VERSION THOMAS LEHNERT ** "));
  Serial.print(lastch);
  Serial.println(" **");
#ifndef AiOPLUS
#ifndef AiO
  ...
  Serial.println();
#ifdef Hardwareserial
  Serial.println(F("HardwareSerial: aktiv")); 
#endif 
  Serial.println();
  // ------------------------------------------------------------------------------------
#ifdef AiO
  ...
  // ------------------------------------------------------------------------------------
#ifndef AiOPLUS
#ifndef AiO                             // Tonuino Klassik Versionen
 #ifdef HW_PowerOff
  ...
  Serial.println ("ShutDown - LOW-active für MOSFET");
  Serial.println ( ); 
  #endif
  #ifndef LowActive
...
#ifdef UsbStick
#ifndef AiOPLUS
#ifndef AiO                             // Nur für Klassik mit Mosfet für USB-Spannung
...
#ifndef AiOPLUS
#ifndef AiO
  digitalWrite(SpkOnPin, LOW);          // Voreinstellung TonUINO Klassik - Speaker Off
#endif
#endif
#ifdef AiO
...
#endif
#ifdef AiOPLUS
  digitalWrite(SpkOnPin, HIGH);         // Voreinstellung AiOPLUS - Speaker Off (Amp Off)
#endif
#endif
  // ----------------------------------------------------------------------------------------
...
myCookie = myCookie +4;                // setzt myCookie um 4 hoch (9 Zusätzliche Shortcuts in den Settings)
 ...
 //************* Ordner des Speichermediums zählen *****************
  ...
  #ifndef Hardwareserial
  if mp3error == true
  Serial.println("mp3-Error ignoriert!");
  #endif
  #endif
  Serial.println ();
#endif
      ...
      && ButtonThree.isPressed())
  {
    ...
    for (int i = 0; i < EEPROM.length(); i++)
    {
      EEPROM.update(i, 0);                          // EEPROM löschen (Klassik und AiOPLUS)
    }
#endif
#ifdef AiO
    for (int i = 0; i < 1024; i++)
...
#ifndef Konsole
  Serial.println ("Konsole reduziert");
#endif
...
ReadMatrix();                                 // Kontrolle ob eine Taste der Matrix gedrückt wurde.
    ...
    if ((ButtonOne.pressedFor(LONG_PRESS)               // 3 Tastendruck Longpress zum Starten Adminmenü
 ...
 // ---- Spezialmodus Von-Bis für Album gibt die Dateinummer relativ zur Startposition wieder ----
        ...
        if (myFolder->mode == 5)                                // Hörbuchmodus
        {
         ...
         currentQueueIndex =0;                                  // Index auf 0 für Track1 setzen
          ...
          mp3.playMp3FolderTrack(340);                             // mp3 340 - Das Hörbuch wird auf den Anfang zurückgesetzt.
                                                                   //           Kapitel 1.
          delay(6000);                                             // Warten bis die Ansage fertig ist.
                                                                   // WaitForTrackToFinish funktioniert hier nicht richtig.
...
/*
    ...
    - TonUINO als Wecker. Ein Uhrenmodul schaltet den TonUINO ein und gibt ein Signal auf Analogpin A6                              - (AiO und TonUINO Klassik)
