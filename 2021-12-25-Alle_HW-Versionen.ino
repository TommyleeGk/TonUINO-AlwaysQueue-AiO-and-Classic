...
#endif
  Serial.println(F("** VERSION THOMAS LEHNERT ** 2021-12-25 **"));
  Serial.println();
#ifndef AiO
...
// ---------------- LongPress -------------------------------------
             ...
             && ignoreButtonOne == false)                       // Langer Druck Pausetaste für Ansage des aktuellen Tracks
    {
      ...
      if (isPlaying())                                          // wenn Wiedergabe läuft
      {
        ...
        if (myFolder->mode == 7 || myFolder->mode == 8)         // Spezialmodus Album oder Spezialmodus Party
        {
...
#endif    
  {
...
#endif    
   {
    ...
    {
      if (isPlaying())                          // wenn Wiedergabe läuft
      if (!ignoreButtonFour)                    // Wenn Taste3 ausgewertet werden soll
      {
        if (mySettings.invertVolumeButtons)     // wenn Funktion der Vol Tasten  getauscht
        {
          volumeUpButton();                     // Lauter-Taste
   ...
   if (ButtonFive.pressedFor(LONG_PRESS))
    ...
    {
      if (isPlaying())                          // wenn Wiedergabe läuft
      if (!ignoreButtonFive)                    // Wenn Taste3 ausgewertet werden soll
      {
        if (mySettings.invertVolumeButtons)     // wenn Funktion der Vol Tasten  getauscht
        {
          volumeDownButton();                   // Leiser-Taste
